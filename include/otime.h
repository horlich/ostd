...
public:
	...
	inline int jj() const { return jz; }

	inline bool istSchaltjahr() const { return Zeit::istSchaltjahr(jz); }
...
public:
	...
	inline int mm() const { return mz; }

	inline const Year& year() const { return jahr; }
...
public:
    ...
    long int epoch() const { return static_cast<long int>(myTime); }

    // Siehe dazu man localtime...
    struct tm localtime() const;

    inline int tt() const { return localTime.tm_mday; }

    inline int mm() const { return localTime.tm_mon + 1; }

    inline int jj() const { return localTime.tm_year + 1900; }

    inline Year getYear() const { return Year(jj()); }

    inline Month getMonth() const { return Month(mm(), getYear()); }

    inline Day getDay() const { return Day(tt(), getMonth()); }
    ...
    inline std::string date10() const { return formatiere("%d.%m.%Y", 10); }

    inline std::string now19() const { return formatiere("%d.%m.%Y %H:%M:%S", 19); }

    inline std::string now8() const { return formatiere("%H:%M:%S", 8); }

    inline std::string timesql16() const { return formatiere("%Y-%m-%d %H:%M", 16); }

    inline std::string timesql19() const { return formatiere("%Y-%m-%d %H:%M:%S", 19); }

    // Timestamp mit Format 8.4
    inline std::string tmstp84() const { return formatiere("%Y%m%d.%H%M", 13); }

    inline std::string getDatum() const { return date10(); }
};
